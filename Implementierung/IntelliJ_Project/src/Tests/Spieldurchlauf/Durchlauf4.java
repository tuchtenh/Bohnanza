...
/**
 * Spieldurchlauf mit nur leichten Bots und angepasstem Deck
 */
public class Durchlauf4 {
    ...
    public static void beforeClass() throws RemoteException, MalformedURLException {
        ...
        for (int i = 0; i < 50; i++) {
            karten.add(new Brechbohne());
        }
        deck = new DeckImpl(karten);
        deck.mischen();
        spiel.setDeck(deck);
    ...
    public void testdurchlauf() throws RemoteException, InterruptedException, MalformedURLException {
        /*
        for (Akteur b : spiel.getSpieler()) {

            spiel.setActPlayer(b);
            //entfernt die Karten aus der Hand, die bei der automatischen Spielerstellung
            //hinzugefügt wurden und fügt neue Karten aus dem Deck in die Hand des Spielers ein:
            for (int i = 0; i < 5; i++) {
                b.removeKarte(0);
                b.addKarte(deck.ziehe());
            }
